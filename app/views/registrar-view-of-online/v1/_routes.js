...
module.exports = function (router) {
  ...
  router.post('/registrar-view-of-online/v1/04-check-answers-page', function (req, res) {
    const id = parseInt(req.body.id, 10)
    ...
    if (selectedOption === 'yes-complete') {
      res.redirect('/registrar-view-of-online/v1/05-sign-page?id=' + id)
    } else if (selectedOption === 'no-an-issue') {
      res.redirect('/registrar-view-of-online/v1/05a-next-steps-contact?id=' + id)
    } else {
  ...
  router.get('/registrar-view-of-online/v1/05-sign-page', function (req, res) {
    ...
    res.render('registrar-view-of-online/v1/05-sign-page', { record, id })
  })

  // POST 05-sign-page – user confirms they want to accept
  router.post('/registrar-view-of-online/v1/05-sign-page', function (req, res) {
    const id = parseInt(req.body.id, 10)

    // store any 'signed' flag here if you need to

    // Go to 06 confirmation for this record
    res.redirect('/registrar-view-of-online/v1/06-confirmation-page?id=' + id)
  })
